import { customFormatterForChart } from "@/lib/humanFormat";
        ...
        <ul className="mt-1" style={{ columnCount: 2 }}>
          {dataKeys.map((key, i) => (
            ...
            >
              <div
                className={
                  (!colors || !colors[i] ? "bg-primary" : "") +
                  " rounded-full w-2 h-2 mr-1"
                }
                style={{
                  backgroundColor: !colors || !colors[i] ? "" : colors[i],
                }}
              />
              {key}
            </li>
          ...
          {payload.map((item: any, i: number) => (
            <li
              key={i}
              className="h-8 flex items-center text-sm font-bold text-foreground"
            >
              {typeof item.value !== "number" || !customFormatter // @ts-ignore
                ? item.value
                : customFormatter(item.value)}
            </li>
          ))}
        </ul>
      </div>
